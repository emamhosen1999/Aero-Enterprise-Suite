...
class ReplayOrphanedBiometricPunches extends Command
{
     ...
     *
     * The eligibility re-check is the important part. A row whose RECORDED reason
     * was recoverable is still moved into "needs configuration review" if the
     * employee's attendance type is non-biometric TODAY — configuration may have
     * changed since the punch was rejected, and replaying against a stale reason
     * would override current policy just as surely as replaying the rows that were
     * rejected for it in the first place.
     *
    ...
    private function bucket(Collection $rows): array
    {
        ...
        foreach ($rows as $row) {
            ...
            if ($reason !== AttendancePunchService::NO_OPEN_RECORD_MESSAGE) {
                $configReview[] = $entry + ['current_reason' => $reason];
    ...
    private function predict(array $group): array
    {
        ...
        if ($first['check_type'] !== 'out') {
            return [
                'gains' => false,
                'outcome' => 'first punch is '.$first['check_type'].', not a plain check-out — not promotable',
            ];
        }
        ...
        if (count($punches) < 2) {
            return [
                'gains' => true,
                'outcome' => 'in '.$in.' (no closing punch — day stays open)',
            ];
        }

        return [
            'gains' => true,
            'outcome' => 'in '.$in.' → out '.end($punches)['moment']->format('H:i'),
        ];
    }
    ...
    {
        $this->newLine();
        $this->line('<comment>RECOVERABLE — a check-out that arrived with no open attendance record</comment>');
        $this->line('  Rejected with: "'.AttendancePunchService::NO_OPEN_RECORD_MESSAGE.'"');
        $this->line('  These are the OUT-first days: the terminal was left in OUT mode, so the day\'s first punch');
        $this->line('  carried status=1. AttendancePunchService now records such a punch as the day\'s check-in when —');
        $this->line('  and only when — that day holds no attendance row at all.');
    ...
    private function reportConfigReview(Collection $entries): void
    {
        ...
        $this->line('  These punches are real: a terminal read these employees and the records are on file. They did not');
        $this->line('  become attendance because the employee was configured for a NON-BIOMETRIC method (WiFi/IP, GPS, QR),');
        $this->line('  or for none at all. That is a policy decision an admin made, not a defect — so this command will not');
        $this->line('  force them through. Manufacturing biometric attendance here would silently override that policy.');
    ...
    private function apply(Collection $recoverable, Collection $groups): int
    {
        ...
        if ($tally['failed'] + $tally['unknown_user'] > 0) {
            $this->newLine();
            $this->warn('  Rows that failed again keep punch_status = failed and will be re-selected by the next run.');
            $this->line('  That is safe — the duplicate check and the promotion rule both refuse a day that already has');
            $this->line('  attendance — but a row that never succeeds needs a human. Check its punch_status_reason.');
        }
